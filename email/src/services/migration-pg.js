...
const customStateStorage = {
  load: async function (fn) {
    ...
    let rows = [];
    try {
      const results = await pg.query('SELECT data FROM migrations')
      rows = results.rows;
    } catch (err) {
      // ok
    }
  ...
  save: async function (set, fn) {
    ...
    await pg.query('CREATE TABLE IF NOT EXISTS migrations (id integer PRIMARY KEY, data jsonb NOT NULL)')
